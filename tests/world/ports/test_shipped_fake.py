...
def test_fake_agent_port_ships_in_src() -> None:
    ...
    assert isinstance(instance, AgentPort), (
        f"FakeAgentPort should be an instance of AgentPort protocol, got {type(instance)}"
    )

    # Scan src/clinicloop/world/ports for test imports
    ports_dir = (
        Path(__file__).parent.parent.parent.parent / "src" / "clinicloop" / "world" / "ports"
    )
    ...
    for py_file in ports_dir.glob("**/*.py"):
        with open(py_file) as f:
            ...
            assert "from tests" not in content, (
                f"File {py_file} imports from tests module, which violates src-only requirement"
            )
