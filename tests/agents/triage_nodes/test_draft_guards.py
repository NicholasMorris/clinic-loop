...
def test_draft_needs_clearance(ruleset, _fixed_key: None) -> None:
    ...
    state_dict = {
        "case_id": "c-001",
        "patient_id": "p-001",
        "redacted_thread": [Turn(role="patient", text="What is my order?")],
...
def test_draft_refuses_non_english(ruleset, _fixed_key: None) -> None:
    ...
    state_dict = {
        ...
        "patient_id": "p-002",
        "redacted_thread": [
            Turn(role="patient", text="我想查询我的订单。")
        ],  # "I want to check my order" in Chinese
        "patient_data_block": build_data_block("我想查询我的订单。"),
        "language": "other",
        "escalation_clear": _mint_clear(
            thread_sha256([{"role": "patient", "text": "我想查询我的订单。"}])
        ),
    }
    ...
    assert update.get("routing_reason") == "language", (
        f"Got routing_reason: {update.get('routing_reason')}"
    )
