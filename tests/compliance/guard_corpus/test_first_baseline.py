...
def test_absent_on_origin_main_passes_and_records_first_baseline(
    ...
    # Monkey-patch Path.cwd() to return tmp_path for this check
    ...
    written_len = len(written_manifest)
    current_len = len(current_manifest)
    assert written_manifest == current_manifest, (
        f"Written manifest doesn't match current; {written_len} vs {current_len}"
    )
